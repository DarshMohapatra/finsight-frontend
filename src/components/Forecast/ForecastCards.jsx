...
export default function ForecastCards({ data, currency, fmt, isHeadlineOnly, isListOnly }) {
  ...
  if (isHeadlineOnly) {
    return (
      <div className="mb-6 sm:mb-8 flex flex-col md:flex-row items-start md:items-center justify-between gap-4 sm:gap-6">
        <div>
        ...
        <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }}
          className={`flex items-center gap-3 sm:gap-5 px-4 sm:px-6 py-3 sm:py-4 rounded-xl border ${bgGrad}`}>
        ...
        {data.top_categories?.map((cat, i) => {
          ...
          return (
            <motion.div key={i} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.2 + (i*0.1) }}
              className="bg-[#0b0e14] border border-white/[0.04] rounded-xl p-4 sm:p-5 hover:bg-white/[0.02] transition-colors">
              ...
              <div>
                <p className="text-xl sm:text-2xl font-bold font-mono text-white tracking-tight">{fmt(cat.predicted, currency)}</p>
                {/* FIXED CLARITY HERE */}
